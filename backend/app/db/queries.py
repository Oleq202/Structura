...
async def add_task(title: str, description: str | None, building_id: int, created_by: int, assigned_to: int):
...
async def update_task(task_id: int, title: str | None, description: str | None, building_id: int | None, created_by: int | None, assigned_to: int | None):
    query = "update tasks set title = $2, description = $3, building_id = $4, created_by = $5, assigned_to = $6 where id = $1"
    await _execute(query, task_id, title, description, building_id, created_by, assigned_to)
